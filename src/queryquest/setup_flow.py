"""Interactive setup flow for provider, model, and API key configuration."""
...
def select_provider(console: Console) -> ProviderConfig:
	"""Display provider choices and return selected provider configuration."""
...
def resolve_api_key(config: ProviderConfig) -> str:
	"""Resolve API key from prompt input, env vars, or sensible local defaults."""
	env_key = find_env_api_key(config.env_key_names)
	if config.name == "ollama":
		# Ollama typically runs locally and does not require a secret key.
...
def run_setup(console: Console) -> dict[str, str]:
	"""Run end-to-end setup and persist selected provider credentials/state."""
